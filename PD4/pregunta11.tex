...
\begin{frame}
	...
	\begin{solution}
		...
		%   \end{bNiceMatrix} & >0   \\
		% \end{align*}
		La matriz $A$ es \alert{simétrica} porque $A=A^{T}$.
	\end{solution}
\end{frame}
...
\begin{frame}
	\begin{solution}
		...
		Con $\omega=1.3$, $x^{\left(0\right)}=0\in\mathbb{R}^{6}$ obtenemos la matriz
		del \alert{método de SOR} $T_{\omega}$ y $c_{\omega}$.
		Y $\rho\left(T_{\omega}\right)<1$ por lo que el
		método converge.
		\begin{minipage}{0.45\textwidth}
			\begin{listing}[H]
				\inputminted[
...
\begin{frame}[c]
	...
	\end{listing}
	Notamos que converge después de $12$ iteraciones
...
\begin{frame}
	\begin{solution}
		\begin{minipage}{0.45\textwidth}
			Ahora veremos con el \alert{método del descenso más rápido}.

			Como $A$ es simétrica y definida positiva, converge con este método.

			Tomando $x^{\left(0\right)}=0\in\mathbb{R}^{6}$.
		\end{minipage}
		\begin{minipage}{0.45\textwidth}
			\begin{listing}[H]
				\inputminted[
...
\begin{frame}[c]
	...
	\end{listing}
	Notamos que converge después de $19$ iteraciones
	a la solución exacta
	\begin{math}
		\begin{pNiceMatrix}
			1 \\
